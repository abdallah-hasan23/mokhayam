...
@section('title','لوحة التحكم') @section('page-title','لوحة التحكم') @section('breadcrumb','الرئيسية')
@section('content')
<div class="pg-head">
  <div class="pg-head-left"><h1>لوحة التحكم</h1><p>{{ now()->isoFormat('dddd، D MMMM YYYY') }} — أهلاً {{ auth()->user()->name }} 👋</p></div>
...
<div class="stat-grid">
  <div class="stat-card sc-gold"><div class="sc-label">إجمالي المقالات</div><div class="sc-value">{{ $stats['total_articles'] }}</div><div class="sc-delta up">↑ {{ $stats['published'] }} منشور</div></div>
  <div class="stat-card sc-green"><div class="sc-label">المشاهدات الكلية</div><div class="sc-value">{{ number_format($stats['total_views']) }}</div><div class="sc-delta up">↑ هذا الشهر: {{ number_format($stats['views_this_month']) }}</div></div>
  <div class="stat-card sc-blue"><div class="sc-label">المشتركون</div><div class="sc-value">{{ number_format($stats['total_subs']) }}</div><div class="sc-delta up">↑ {{ $stats['new_subs'] }} جديد هذا الشهر</div></div>
  <div class="stat-card sc-red"><div class="sc-label">بانتظار المراجعة</div><div class="sc-value">{{ $stats['pending_review'] }}</div>
    <div class="sc-delta {{ $stats['pending_review'] > 0 ? 'dn' : 'neutral' }}">
      @if($stats['pending_review'] > 0)<a href="{{ route('dashboard.articles.index',['status'=>'review']) }}" style="color:var(--red)">راجع الآن ←</a>@else لا يوجد معلّق ✓@endif
    </div>
  </div>
</div>
...
<div class="g2">
  ...
  <div class="card">
    ...
    <div class="act-item">
      <div class="act-dot {{ match($art->status){'published'=>'act-dot-green','review'=>'act-dot-gold','draft'=>'act-dot-gray',default=>'act-dot-red'} }}"></div>
      <div class="act-body">
        <div class="act-text"><strong>{{ $art->user->name }}</strong> {{ match($art->status){'published'=>'نشر','review'=>'أرسل للمراجعة','draft'=>'حفظ مسودة',default=>''} }} «{{ Str::limit($art->title,35) }}»</div>
